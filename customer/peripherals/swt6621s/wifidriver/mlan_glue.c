        ...
        IN t_void *pcontext)
{
     ...
     * and raise RT_ASSERT in *timer_start if this is not the case.
     */
...
int mlan_subsys_init(void)
{
    ...
    wifi_w("mlan_dev.bss_attr[1].bss_num = %d\n", mlan_dev.bss_attr[1].bss_num);
                                      ...
                                      void *priv)
{
    ...
    cmd->result = 0x0;
    wifi_w("%s %d bss_type: %d, pioctl_req->bss_index: %d, cmd_oid: 0x%x\n", __func__, __LINE__, bss_type, pioctl_req->bss_index, cmd_oid);
    if (bss_type == MLAN_BSS_TYPE_UAP)
    {
...
int wifi_nxp_send_assoc(nxp_wifi_assoc_info_t *assoc_info)
{
    ...
    int size_assoc = sizeof(struct skw_assoc_req_param) + assoc_info->wpa_ie.ie_len;
    struct skw_assoc_req_param *params_assoc = rt_malloc(size_assoc);
    wifi_w("%s %d size_assoc=%d params=%p\n", __func__, __LINE__, size_assoc, params_assoc);
    rt_memset(params_assoc, 0, size_assoc);
    if (assoc_info->wpa_ie.ie_len)
    {
        rt_memcpy(params_assoc->req_ie, assoc_info->wpa_ie.ie, assoc_info->wpa_ie.ie_len);
        params_assoc->req_ie_len = assoc_info->wpa_ie.ie_len;
        params_assoc->req_ie_offset = sizeof(struct skw_assoc_req_param);
    }
    else
    {
        params_assoc->req_ie_len = 0;
    }
    wifi_send_bss_ioctl(&bss);
    int ret_assoc = skw_cmd_assoc(params_assoc);
    rt_free(params_assoc);
    return ret_assoc;
#else
    wifi_e("%s %d assoc_info=%p \n", __func__, __LINE__, assoc_info);
    RT_ASSERT(0);
    return wifi_send_bss_ioctl(&bss);
#endif
}
...
int wifi_process_cmd_response(struct skw_msg *msg)//CMD ack的处理函数
{
    mlan_private *pmpriv          = (mlan_private *)mlan_adap->priv[0];
    mlan_status rv                = MLAN_STATUS_SUCCESS;
    enum wifi_event_reason result = WIFI_EVENT_REASON_FAILURE;
    t_u8 *sta_addr;
    HostCmd_DS_COMMAND *resp = wifi_get_command_buffer();
    t_u16 command = (resp->command & HostCmd_CMD_ID_MASK);//当前发送的指令
    wifi_w("%s %d command=%d msg->id=%d\n", __func__, __LINE__, command, msg->id);
#ifdef RW610
...
int wifi_config_bgscan_and_rssi(const char *ssid)
{
    ...
    strncpy((char *)pmpriv->scan_cfg.ssid_list[0].ssid, (char *)ssid, MLAN_MAX_SSID_LENGTH - 1);
    pmpriv->scan_cfg.ssid_list[0].ssid[MLAN_MAX_SSID_LENGTH - 1] = '\0';
...
int wifi_handle_fw_event(struct skw_msg *msg)
{
    mlan_private *pmpriv     = (mlan_private *)mlan_adap->priv[0];
...
#endif
    /*
    Event_Ext_t结构体类型需要改为希微的msg结构体类型
    */
    /* Event_Ext_t结构体类型需要改为希微的msg结构体类型
       这里的msg是从 wifi_event_completion 函数中传入的
    */
    struct skw_msg *evt = msg;//evt->event_id  收到的event的类型
...
#else
    wevt_d("EVENT - : 0x%x Len : %d Reason: %d", evt->type, evt->total_len, evt->id);
#endif
#if 0
...
#endif
    wifi_w("%s %d len=%x evt->type=%d\n", __func__, __LINE__, evt->total_len, evt->id);
    switch (evt->id)
    {
...
#endif
        break;
    case EVENT_DEAUTHENTICATED://// 去认证帧
#if CONFIG_WPA_SUPP
        wifi_if_ctx_rtos->associated = MFALSE;
#endif
        if (evt->id == 0U)
    ...
    case EVENT_MIC_ERR_UNICAST:
        ...
        break;
    case EVENT_BG_SCAN_REPORT://后台扫描报告
    ...
    case EVENT_DELBA:
    {
        wlan_11n_delete_bastream((mlan_private *)mlan_adap->priv[evt->inst_id], (t_u8 *)&evt->id);
    }
    break;
    ...
    case EVENT_CHANNEL_SWITCH:
    {
        ...
        MrvlIEtypes_channel_band_t *tlv = (MrvlIEtypes_channel_band_t *)&evt->resv;

        pecsa_info->bss_type    = evt->type;
    ...
    break;
    case EVENT_MICRO_AP_STA_ASSOC://关联
    {
        /*
        ...
        //event_sta_addr = (t_u8 *)&evt->mac_addr;
    ...
    case EVENT_BLOCKEDSTA_AUTH_REPORT:
        ...
        break;
#if CONFIG_EXT_SCAN_SUPPORT
    case 11://SKW_EVENT_SCAN_REPORT://EVENT_EXT_SCAN_REPORT:
        //pext_scan_result = (mlan_event_scan_result *)((t_u8 *)msg->data + 4);
        wifi_w("%s %d msg+8=%p msg=%p\n", __func__, __LINE__, (t_u8 *)msg + 8, msg);
        rv = wlan_handle_event_ext_scan_report(pmpriv, (t_u8 *)msg + 8); //解析beacon帧
...
#endif
        // pmpriv->assoc_req_size = evt->len - 8;
...
#endif
        break;
    case SKW_EVENT_CREDIT_UPDATE:
        /* Credit update event */
        wifi_w("%s %d SKW_EVENT_CREDIT_UPDATE\n", __func__, __LINE__);
        break;
    case SKW_EVNET_RX_MGMT://04
        /* Received mgmt frame event */
        wifi_w("%s %d SKW_EVNET_RX_MGMT len=%d\n", __func__, __LINE__, msg->total_len);

        int wifi_auth_event_req(const uint8_t interface, const uint8_t *buffer, const uint16_t len);
        wifi_auth_event_req(0, (const uint8_t *)msg, msg->total_len);//sta
        break;
    case SKW_EVENT_TX_FRAME://0x19
        /* Transmit frame event */
        wifi_w("%s %d SKW_EVENT_TX_FRAME len=%d\n", __func__, __LINE__, msg->total_len);
        //等待处理
        break;
    case SKW_EVENT_BA_ACTION: //13

        break;
    default:
        // wifi_d("Event 0x%x not implemented", evt->event_id);
        wifi_w("%s %d Event 0x%x not implemented\n", __func__, __LINE__, evt->id);
...
// void *wifi_scan_get_all_results(void)
// {
//     mlan_private *pmpriv = (mlan_private *)mlan_adap->priv[0];
//     mlan_adapter *pmadapter = pmpriv->adapter;
//     return (void*)pmadapter->pscan_table[0];
// }
...
void wifi_prepare_get_value1(HostCmd_DS_COMMAND *cmd, t_u16 seq_number)
{
    ...
    cmd->params.mac_reg.offset = 0;//wlan_cpu_to_le16((t_u16)WLAN_VALUE1);
...
#include "skw_type_msg.h"
void _wifi_set_mac_addr(const uint8_t *mac, mlan_bss_type bss_type)//设置本机的mac地址
{
