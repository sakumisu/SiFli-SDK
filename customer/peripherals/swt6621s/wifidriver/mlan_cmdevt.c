                             ...
                             IN t_void *pdata_buf)
{
    ...
    */
    ...
    rt_kprintf("%s %d n_channels=%d pdata_buf=%p\n", __func__, __LINE__, test_scan_req.n_channels, pdata_buf);
...
mlan_status wlan_cmd_get_hw_spec(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *pcmd)
{
    ...
    //(void)__memcpy(pmpriv->adapter, hw_spec->permanent_addr, skw->address.addr[0], MLAN_MAC_ADDR_LENGTH);
 ...
 */
/*
函数的作用是 处理从固件返回的硬件规格（HW Specification）命令响应，并将相关信息存储到驱动程序的内部数据结构中。
该函数通常在设备初始化过程中调用，用于获取设备的硬件能力和配置
*/
mlan_status wlan_ret_get_hw_spec(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *resp, IN t_void *pioctl_buf)
{
    ...
    rt_kprintf("%s %d %s: fw_bands=0x%X,pmadapter->priv_num=%d\n", __func__, __LINE__, pmadapter->fw_bands, pmadapter->priv_num);
...
void wlan_skw_htinfo(BSSDescriptor_t *pscan_table, struct skw_join_param *params)
{
    uint8_t ht_oper   = pscan_table->pht_info->ht_info.field2;
    uint8_t pri_chan = pscan_table->pht_info->ht_info.pri_chan;
    rt_kprintf("%s %d ht_oper->field1=%d\n", __func__, __LINE__, ht_oper);
    params->center_chn2 = 0;
    switch (ht_oper & 0x3)
    {
    case IEEE80211_HT_PARAM_CHA_SEC_NONE://0
        params->bandwidth = SKW_CHAN_WIDTH_20;
        params->center_chn1 = pri_chan;

        break;

    case IEEE80211_HT_PARAM_CHA_SEC_ABOVE://1
        params->bandwidth = SKW_CHAN_WIDTH_40;
        params->center_chn1 = pri_chan + 2;
        break;

    case IEEE80211_HT_PARAM_CHA_SEC_BELOW://3
        params->bandwidth = SKW_CHAN_WIDTH_40;
        params->center_chn1 = pri_chan - 2;
        break;
...
mlan_status wlan_cmd_tx_frame(pmlan_private pmpriv, HostCmd_DS_COMMAND *cmd, t_u16 cmd_action, t_void *pdata_buf)
{
    ...
    int i = 0;
    for (i = 0; i <  pmpriv->adapter->num_in_scan_table; i++)
    {
        if (strncmp(ssid, pmpriv->adapter->pscan_table[i].ssid.ssid, MLAN_MAX_SSID_LENGTH) == 0)
        {
            rt_kprintf("found ssid=%s i=%d\n", ssid, i);
            break;
        }
    }
    BSSDescriptor_t *pscan_table = &pmpriv->adapter->pscan_table[i];
    int size = sizeof(struct skw_join_param) + pscan_table->ies_len;
    size = size % 2 ? size + 1 : size;
    params = rt_malloc(size);
    rt_kprintf("%s %d ssid=%s sizeof(struct skw_join_param)=%d,params=%p size=%d\n", __func__, __LINE__,
               ssid, sizeof(struct skw_join_param), params, size);

    rt_memset(params, 0x00, size);
    ENTER();

    params->chan_num             = pscan_table->channel;// 信道号（如1、6、36等）
    params->band                 = 0;//pscan_table->bss_band;// 频段（2.4G/5G/6G等，参考enum SKW_BAND）
    params->beacon_interval      = pscan_table->beacon_period; // Beacon 间隔（单位：TU，通常为100）
    params->capability           = pscan_table->cap_info;// 能力信息字段（802.11标准定义，关联帧中的capability字段）
    params->bssid_index          = 0;
    params->max_bssid_indicator  = 0;

    rt_memcpy(params->bssid, pscan_table->mac_address, MLAN_MAC_ADDR_LENGTH);
    params->roaming              = 0;
    params->reserved             = 0;
    params->bss_ie_offset        = sizeof(struct skw_join_param);
    params->bss_ie_len           = pscan_table->ies_len;
    rt_kprintf("%s %d len=%d\n", __func__, __LINE__, params->bss_ie_len);
    // params->bss_ie = rt_malloc(pscan_table->ies_len);
    rt_memcpy(params->bss_ie, pscan_table->ies, pscan_table->ies_len);
    wlan_skw_htinfo(pscan_table, params);
    int skw_cmd_join(struct skw_join_param * params);
    skw_cmd_join(params);

    struct cfg80211_auth_request *req = (struct cfg80211_auth_request *)cmd;
    req->auth_type = pmpriv->auth_alg;
    int skw_cmd_auth(struct cfg80211_auth_request * req);

    skw_cmd_auth(req);

    //RT_ASSERT(0);
    //rt_free(params->bss_ie);
    rt_free(params);
