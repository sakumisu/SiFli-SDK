...
{
    t_u32 key_disable;        // 是否禁用该密钥（MTRUE=禁用，MFALSE=启用）
    t_u32 key_remove;         // 是否移除该密钥（MTRUE=移除，仅检查key_index）
    t_u32 key_index;          // 密钥索引（如WEP的0~3，或组播/单播密钥索引）
    bool  is_current_wep_key; // 是否为当前WEP密钥（仅WEP模式有效）
    t_u32 key_len;            // 密钥长度（字节）
    t_u8  key_material[MLAN_MAX_KEY_LENGTH]; // 密钥内容（最大32字节）
    t_u8  mac_addr[MLAN_MAC_ADDR_LENGTH];    // 关联的MAC地址（单播密钥时为对端MAC，组播时为0xff:ff:ff:ff:ff:ff）
    t_u32 is_wapi_key;        // 是否为WAPI密钥（一般为0，WAPI专用）
    t_u8  pn[PN_SIZE];        // 初始包号（Packet Number，TKIP/CCMP/GCMP等用作加密计数器）
    t_u32 key_flags;          // 密钥标志位（如组播/单播、TX/RX、默认密钥等，见下表）
