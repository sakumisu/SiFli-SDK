 ...
 */
/*
当前文件 wifi-imu.c 的作用是实现与 Wi-Fi 模块相关的驱动程序功能，主要用于管理 Wi-Fi 模块的初始化、通信和控制。
它提供了与底层硬件（如 IMU 通信接口）和上层协议栈之间的接口*/
#include <mlan_api.h>
#ifndef RW610
...
hal_imumc_status_t imumc_cmdrsp_handler(IMU_Msg_t *pImuMsg, uint32_t length)
{
    RT_ASSERT(NULL != pImuMsg);
    RT_ASSERT(0 != length);
    RT_ASSERT(IMU_MSG_COMMAND_RESPONSE == pImuMsg->Hdr.type);
...
hal_imumc_status_t imumc_event_handler(IMU_Msg_t *pImuMsg, uint32_t length)
{
    RT_ASSERT(NULL != pImuMsg);
    RT_ASSERT(0 != length);
    RT_ASSERT(IMU_MSG_EVENT == pImuMsg->Hdr.type);
...
hal_imumc_status_t imumc_rxpkt_handler(IMU_Msg_t *pImuMsg, uint32_t length)
{
    ...
    RT_ASSERT(NULL != pImuMsg);
    RT_ASSERT(0 != length);
    RT_ASSERT((IMU_MSG_RX_DATA == pImuMsg->Hdr.type) || (IMU_MSG_MULTI_RX_DATA == pImuMsg->Hdr.type));
...
hal_imumc_status_t imumc_ctrl_handler(IMU_Msg_t *pImuMsg, uint32_t length)
{
    ...
    RT_ASSERT(NULL != pImuMsg);
    RT_ASSERT(IMU_MSG_CONTROL == pImuMsg->Hdr.type);
