 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_ret_802_11_associate(IN mlan_private *pmpriv, IN IEEEtypes_MgmtHdr_t *assc_resp, IN t_void *pioctl_buf)
{
    mlan_status ret            = MLAN_STATUS_SUCCESS;
    ...
    ENTER();
    rt_kprintf("%s %d host_mlme=%d\n", __func__, __LINE__, pmpriv->curr_bss_params.host_mlme);
    if (pmpriv->curr_bss_params.host_mlme)
    {
        hdr = (IEEEtypes_MgmtHdr_t *)assc_resp;
        rt_kprintf("%s %d hdr->BssId="MACSTR"\n", __func__, __LINE__, MAC2STR(hdr->BssId));
        rt_kprintf("%s %d pmpriv->pattempted_bss_desc->mac_address="MACSTR"\n", __func__, __LINE__,
                 MAC2STR(pmpriv->pattempted_bss_desc->mac_address));
        if (!__memcmp(pmpriv->adapter, hdr->BssId, pmpriv->pattempted_bss_desc->mac_address, MLAN_MAC_ADDR_LENGTH))
        {
            passoc_rsp = (IEEEtypes_AssocRsp_t *)((t_u8 *)assc_resp + sizeof(IEEEtypes_MgmtHdr_t));
        }
        else
        {
            passoc_rsp = (IEEEtypes_AssocRsp_t *)assc_resp;
        }
    }
    else
        passoc_rsp = (IEEEtypes_AssocRsp_t *)(void *)assc_resp;
