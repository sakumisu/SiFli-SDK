        ...
        IN ChanScanParamSet_t *pscan_chan_list)
{
    ...
    ENTER();
    struct cfg80211_scan_request test_scan_req = *(struct cfg80211_scan_request *)pscan_cfg_out->tlv_buf;
    PRINTM(MINFO, "%s %d n_channels=%d,n_ssids=%d\n", __func__, __LINE__, test_scan_req.n_channels, test_scan_req.n_ssids);
    ...
    if (!pscan_chan_list->chan_number)
    {
        ...
        LEAVE();
        PRINTM(MINFO, "%s %d pscan_chan_list->chan_number=%d\n", __func__, __LINE__, pscan_chan_list->chan_number);
        return MLAN_STATUS_FAILURE;
    }
    ...
    do //while (ptmp_chan_list->chan_number != 0U)//一次性发出
    {
        tlv_idx                   = 0;
        total_scan_time           = 0;
        ...
        PRINTM(MINFO, "%s %d cmd_no=0x%x pscan_cfg->tlv_buf=%p\n", __func__, __LINE__, cmd_no, pscan_cfg_out->tlv_buf);
        for (int i = 0; i < pscan_cfg_out->tlv_buf_len; i++)
        {
            if (i % 16 == 0) PRINTM(MINFO, "\n");
            if (i % 8 == 0) PRINTM(MINFO, " ");
            // /PRINTM(MINFO,"%02x ",*((t_u8 *)pscan_cfg_out->tlv_buf + i));
            PRINTM(MINFO, "%02x ", ((t_u8 *)&test_scan_req + i));
        }
        PRINTM(MINFO, "\n");
        struct cfg80211_scan_request test_scan_req_1 = *(struct cfg80211_scan_request *)pscan_cfg_out->tlv_buf;
        PRINTM(MINFO, "%s %d n_channels=%d\n", __func__, __LINE__, test_scan_req_1.n_channels);
        ret = wlan_prepare_cmd(pmpriv, (t_u16)cmd_no, HostCmd_ACT_GEN_SET, 0, pioctl_buf, &test_scan_req);//pscan_cfg_out 要发送的数据
        PRINTM(MINFO, "%s %d ret=%x\n", __func__, __LINE__, ret);
        ...
        if (abort_split_scan)
        {
            ...
            for (i = 0; i < pmadapter->num_in_scan_table; i++)
            {
                ...
                {
                    PRINTM(MINFO, "%s %d bss_entry->ies=%p\n", __func__, __LINE__, bss_entry->ies);
    ...
    while (0); //(ptmp_chan_list->chan_number != 0U);
        ...
        OUT t_u8 *pscan_current_only)
{
    ...
    struct cfg80211_scan_request *scan_req = MNULL;

    ENTER();

    pmpriv->ssid_filter = MFALSE;//要过滤的SSID
    ...
    scan_req = (struct cfg80211_scan_request *)pscan_cfg_out->tlv_buf;
    ...
    *pscan_current_only = MFALSE;
    ...
    if (puser_scan_in != MNULL)
    {
        ...
        PRINTM(MINFO, "%s %d NELEMENTS(puser_scan_in->ssid_list)=%d\n", __func__, __LINE__, NELEMENTS(puser_scan_in->ssid_list));
        //RT_ASSERT(0);
#if 1
#if 0
...
#endif
        scan_req->n_ssids = ssid_idx + 1;
#else
        req.n_ssids = 1;//NELEMENTS(puser_scan_in->ssid_list);
#endif

        /*
            命令中发送的默认通道数较低，
            以确保固件的响应缓冲区不会截断扫描结果。.
            这不是固件中应用于扫描结果的SSID或BSSID过滤器的问题。
         */
    ...
    *ppchan_list_out = (MrvlIEtypes_ChanListParamSet_t *)(void *)scan_req;
    PRINTM(MINFO, "%s %d *ppchan_list_out=%p,puser_scan_in=%p,chan_number=%d\n", __func__, __LINE__, *ppchan_list_out, puser_scan_in,
    ...
    if ((puser_scan_in != MNULL) && puser_scan_in->chan_list[0].chan_number)
    {
        ...
        for (chan_idx = 0; chan_idx < WLAN_USER_SCAN_CHAN_MAX && puser_scan_in->chan_list[chan_idx].chan_number;
                chan_idx++)
        {
            radio_type = puser_scan_in->chan_list[chan_idx].radio_type;//信道频段
#if 1
            scan_req->channels[chan_idx].radio_type = radio_type;
            scan_req->channels[chan_idx].chan_number = puser_scan_in->chan_list[chan_idx].chan_number;
            scan_req->channels[chan_idx].scan_type = 1;
            scan_req->channels[chan_idx].scan_time = puser_scan_in->chan_list[chan_idx].scan_time;
#endif
            PRINTM(MINFO, "%s %d radio_type=%d,pmpriv->config_bands=%d\n", __func__, __LINE__, radio_type, pmpriv->config_bands);
            pmpriv->config_bands = BAND_B | BAND_G;//0U;
            ...
            PRINTM(MINFO, "%s %d channel=%d radio_type=%d\n", __func__, __LINE__, channel, radio_type);
            (pscan_chan_list + chan_idx)->chan_number = channel;//信道号
...
#endif
            PRINTM(MINFO, "%s %d\n", __func__, __LINE__);
            // for(int i=0;i<sizeof(struct cfg80211_scan_request);i++)
            // {
            //     if(i%16==0) PRINTM(MINFO,"\n");
            //     if(i%8==0) PRINTM(MINFO," ");
            //     PRINTM(MINFO,"%02x ",*((t_u8*)pscan_cfg_out->tlv_buf+i));
            // }
            PRINTM(MINFO, "%s %d chan_idx=%d radio_type =%d,chan_number=%d,scan_type=%d,scan_time=%d\n", __func__, __LINE__,
                   chan_idx, scan_req->channels[chan_idx].radio_type,
                   scan_req->channels[chan_idx].chan_number, scan_req->channels[chan_idx].scan_type,
                   scan_req->channels[chan_idx].scan_time);
            PRINTM(MINFO, "\n");
        }
        ...
        scan_req->n_channels = chan_idx + 1;
    }
    else
    {
        ...
        IN t_u8 ext_scan)
{
    ...
    t_u8 *pcurrent_ptr = (t_u8 *)pbeacon_info;
    PRINTM(MINFO, "%s %d pcurrent_ptr=%p pcurrent_ptr=%p\n", __func__, __LINE__, pbeacon_info, pcurrent_ptr);
    t_u8 *prate;
    t_u8 element_len;
    t_u16 total_ie_len;
    t_u8 bytes_to_copy;
    ...
    t_u32 bytes_left_for_current_beacon = *bytes_left;
    ...
    rate_size          = 0;
    beacon_size        = 0;
    PRINTM(MINFO, "%s %d pcurrent_ptr=%p bytes_left_for_current_beacon=%d\n", __func__, __LINE__, pcurrent_ptr, bytes_left_for_current_beacon);

    pcurrent_ptr += 16;//跳过    le16 frame_control;  le16 duration;
    bytes_left_for_current_beacon -= 16;
    ...
    PRINTM(MINFO, "%s %d fixed_ie.beacon_interval=%d\n", __func__, __LINE__, pbss_entry->beacon_period);
    pcurrent_ptr += 2;
    bytes_left_for_current_beacon -= 2U;

    /* Capability information is 2 bytes long  能力信息*/
    ...
    while (bytes_left_for_current_beacon >= 2U)
    {
        ...
        PRINTM(MINFO, "%s %d element_id=%d,element_len=%d,total_ie_len=%d,bytes_left_for_current_beacon=%d\n", __func__, __LINE__, element_id, element_len,
               total_ie_len, bytes_left_for_current_beacon);
        if (bytes_left_for_current_beacon < total_ie_len)
        {
            PRINTM(MINFO, "InterpretIE: Error in processing IE, bytes left < IE length\n");
        ...
        {
        case SSID:
            if ((element_len > MRVDRV_MAX_SSID_LENGTH) || (element_len == 0))
            {
                bytes_left_for_current_beacon = 0;
                ret = MLAN_STATUS_FAILURE;
                PRINTM(MINFO, "%s %d element_len=%d\n", __func__, __LINE__, element_len);
                goto end;
            }
            char *ssid = pcurrent_ptr + 2;
            //判断ssid是否为null
            for (int i = 0; i < element_len; i++)
            {
                ...
                {
                    PRINTM(MINFO, "InterpretIE: NULL SSID\n");
                    pbss_entry->ssid.ssid_len = 0;
                    memset(pbss_entry->ssid.ssid, 0, sizeof(pbss_entry->ssid.ssid));
                    PRINTM(MINFO, "InterpretIE: SSID cleared\n");
                    ret = MLAN_STATUS_FAILURE;
                    bytes_left_for_current_beacon = 0;
                    goto end;
                }
            ...
            if (1)//(pmadapter->wpa_supp_scan_triggered == MTRUE)
            {
                wifi_d("Alloc ies for BSS");
                //如果是wpa_supplicant扫描，分配内存
                //并把当前的beacon信息拷贝到分配的内存中
                PRINTM(MINFO, "%s %d bytes_left_for_current_beacon=%d\n", __func__, __LINE__, bytes_left_for_current_beacon);
                ...
                PRINTM(MINFO, "%s %d pbss_entry->ies=%p %x %x %x\n", __func__, __LINE__, pbss_entry->ies, pcurrent_ptr[0], pcurrent_ptr[1], pcurrent_ptr[2]);
            ...
            __memset(pmadapter, pbss_entry->ssid.ssid, 0, MRVDRV_MAX_SSID_LENGTH);
            (void)__memcpy(pmadapter, pbss_entry->ssid.ssid, (pcurrent_ptr + 2), element_len);
            wscan_d("AP: %s", pbss_entry->ssid.ssid);
            PRINTM(MINFO, "InterpretIE: ssid: %-32s element_len=%d\n", pbss_entry->ssid.ssid, element_len);
            break;
...
static t_void wlan_scan_skw_data_packing(IN mlan_private *pmpriv, IN const wlan_user_scan_cfg *puser_scan_in,
        OUT wlan_scan_cmd_config *pscan_cfg_out)
{
    struct skw_scan_chan_info *chan;
    int size, nssids_size, offset = 0;
    u16 roam_chn = 0;
    u16 scan_chn_num = 0;
    char *buff = NULL;
    struct skw_scan_param *param = NULL;
    size = sizeof(struct skw_scan_param) +
           puser_scan_in->chan_list * sizeof(*chan) +
           NELEMENTS(puser_scan_in->ssid_list) * sizeof(mlan_802_11_ssid) +
           /*req->extra_ies_len +*/ 0;
                               ...
                               IN const wlan_user_scan_cfg *puser_scan_in)
{
    ...
    struct cfg80211_scan_request test_scan_req = *(struct cfg80211_scan_request *)&pscan_cfg_out->config;
    PRINTM(MINFO, "%s %d n_channels=%d\n", __func__, __LINE__, test_scan_req.n_channels);
...
mlan_status wlan_cmd_802_11_scan_ext(IN mlan_private *pmpriv, IN HostCmd_DS_COMMAND *pcmd, IN t_void *pdata_buf)
{
    ...
    struct cfg80211_scan_request test_scan_req = *(struct cfg80211_scan_request *)pdata_buf;
    PRINTM(MINFO, "%s %d n_channels=%d n_ssids=%d\n", __func__, __LINE__, test_scan_req.n_channels, test_scan_req.n_ssids);
    //rt_memcpy((void *)&test_scan_req, (struct cfg80211_scan_request *)pdata_buf, sizeof(struct cfg80211_scan_request));
    int wifi_start_scan(struct cfg80211_scan_request * req);
    wifi_start_scan((struct cfg80211_scan_request *)pdata_buf);
 ...
 *  @return             MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static uint32_t scan_req_numm = 0;
        ...
        IN t_u16 scan_resp_size)
{
    ...
    struct skw_mgmt_hdr *ptlv                    = MNULL;
    ...
    num_in_table = pmadapter->num_in_scan_table;
    ptlv         = (struct skw_mgmt_hdr *)pscan_resp;
    bytes_left = ptlv->mgmt_len;
    PRINTM(MINFO, "%s %d bytes_left=%d num_in_table=%d\n", __func__, __LINE__, bytes_left, num_in_table);
    /*

     * 处理每个扫描响应返回的number_of_sets。将信息保存在 bss_new_entry 中，
     * 然后作为对现有条目的更新或作为表末尾的添加插入到驱动程序扫描表中
     */
#if !CONFIG_MEM_POOLS
    ret = pcb->moal_malloc(pmadapter->pmoal_handle, sizeof(BSSDescriptor_t), MLAN_MEM_DEF, (t_u8 **)&bss_new_entry);
...
#endif
        PRINTM(MINFO, "%s %d bytes_left=%d \n", __func__, __LINE__, bytes_left);
        /* Zero out the bss_new_entry we are about to store info in */
        (void)__memset(pmadapter, bss_new_entry, 0x00, sizeof(BSSDescriptor_t));

        /* Process the data fields and IEs returned for this BSS  解析扫描到的 信标帧 beacon*/
        PRINTM(MINFO, "%s %d ptlv->mgmt[0]=%p \n", __func__, __LINE__, &ptlv->mgmt[0]);
        if (wlan_interpret_bss_desc_with_ie(pmadapter, bss_new_entry, (t_u8 **)&ptlv->mgmt[0], &bytes_left, MTRUE) == //解析scan的管理帧
                MLAN_STATUS_SUCCESS)
        {
...
#endif
            band = radio_type_to_band(ptlv->band);
            if (bss_new_entry->channel == 0)
                bss_new_entry->channel = ptlv->chan;
            /* Save the band designation for this entry for use in join */
            bss_new_entry->bss_band = band;
            cfp = wlan_find_cfp_by_band_and_channel(pmadapter, bss_new_entry->bss_band, (t_u16)bss_new_entry->channel);

            PRINTM(MINFO, "%s %d bss_new_entry->channel=%d bss_new_entry->bss_band=%d \n", __func__, __LINE__,
                   bss_new_entry->channel, bss_new_entry->bss_band);
            ...
            if (pmpriv->ssid_filter)
            {
                ...
                if (idx2 == NELEMENTS(pmpriv->filter_ssid))
                {
                    ...
                    if (bss_new_entry->ies != NULL)
                    {
                        PRINTM(MINFO, "%s %d bss_new_entry->ies=%p \n", __func__, __LINE__, bss_new_entry->ies);
                        OSA_MemoryFree(bss_new_entry->ies);
                        bss_new_entry->ies = NULL;
                    }
#endif
             ...
             *   table
             */
            PRINTM(MINFO, "%s %d bss_idx=%d num_in_table=%d\n", __func__, __LINE__, bss_idx, num_in_table);
            if (bss_idx == num_in_table)
            {
                /* Range check the bss_idx, keep it limited to the last entry */
                if (bss_idx == MRVDRV_MAX_BSSID_LIST)
                {
                    lowest_rssi_index = wlan_find_worst_network_in_list(pmadapter->pscan_table, MRVDRV_MAX_BSSID_LIST);//排序
                }
                else
                {
                    num_in_table++;
            ...
            PRINTM(MINFO, "%s %d bss_idx=%d \n", __func__, __LINE__, bss_idx);
            if (bss_idx == MRVDRV_MAX_BSSID_LIST)
            {
                if (pmadapter->pscan_table[lowest_rssi_index].rssi > bss_new_entry->rssi)
                {
...
#endif
                    PRINTM(MINFO, "%s %d lowest_rssi_index=%d \n", __func__, __LINE__, lowest_rssi_index);
                    (void)__memcpy(pmadapter, &pmadapter->pscan_table[lowest_rssi_index], bss_new_entry,
                                   sizeof(pmadapter->pscan_table[lowest_rssi_index]));
                    adjust_pointers_to_internal_buffers(&pmadapter->pscan_table[lowest_rssi_index], bss_new_entry);
                }
    ...
    if (bss_new_entry)
    {
        PRINTM(MINFO, "%s %d bss_new_entry=%p \n", __func__, __LINE__, bss_new_entry);
        //pcb->moal_mfree(pmadapter->pmoal_handle, (t_u8 *)bss_new_entry);
    }
#endif
...
mlan_status wlan_handle_event_ext_scan_report(IN mlan_private *pmpriv, IN t_u8 *pmbuf)
{
    ...
    struct skw_mgmt_hdr *pevent_scan = (struct skw_mgmt_hdr *)(pmbuf);
    //t_u8 *ptlv                          = (pmbuf + sizeof(mlan_event_scan_result));
    t_u16 tlv_buf_left                  = pevent_scan->mgmt_len; //wlan_cpu_to_le16(pevent_scan->buf_size);

    ret = wlan_parse_ext_scan_result(pmpriv, 1, pmbuf, tlv_buf_left);
