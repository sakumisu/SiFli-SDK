                       ...
                       const bool active_scan_triggered)
{
    ...
    wifi_w("%s %d wm_wifi.wpa_supp_scan=%d\n", __func__, __LINE__, wm_wifi.wpa_supp_scan);
...
#endif
    wifi_w("%s %d chan_list[0].radio_type=0x%x num_channels=%d\n", __func__, __LINE__, chan_list[0].radio_type, num_channels);
    ...
    else if (num_channels > 0U && num_channels <= WLAN_USER_SCAN_CHAN_MAX && chan_list != MNULL)
    {
        for (i = 0; i < num_channels; i++)
        {
            ...
            wifi_w("%s %d chan_number=%d\n", __func__, __LINE__, user_scan_cfg->chan_list[i].chan_number);
...
#endif
    wifi_w("%s %d wm_wifi.g_user_scan_cfg=%p\n", __func__, __LINE__, wm_wifi.g_user_scan_cfg);
    if (0)//(wm_wifi.g_user_scan_cfg != NULL)
    {
#if !CONFIG_MEM_POOLS
                 ...
                 unsigned int flags)
{
    ...
    void skw_wifi_set_key(int cipher, u8 key_idx, int key_type,
                          const u8 * key, int key_len, const u8 * addr);
    skw_wifi_set_key(flags, key_index, is_pairwise, key, key_len, mac_addr);
...
int wifi_remove_key(int bss_index, bool is_pairwise, const uint8_t key_index, const uint8_t *mac_addr)
{
    ...
    skw_wifi_del_key(key_index, is_pairwise, 0, mac_addr);
    return 0;//wifi_send_key_material_cmd(bss_index, &sec);
}
...
static int wifi_send_rf_antenna_cmd(t_u16 action, wifi_antcfg_t *wifi_antcfg)
{
    ...
    cmd->seq_num = 0x0;
    cmd->result  = 0x0;
    wifi_w("%s %d %d\n", __func__, action, ant_cfg_1x1.antenna);
    mlan_status rv =
                                       ...
                                       const t_u16 sae_data_len)
{
    ...
    if (1)//(pmadapter->cmd_tx_data == 1U)
    {
        (void)wifi_get_command_lock();
        struct cfg80211_auth_request auth = {0};//(struct cfg80211_auth_request *)wifi_get_command_buffer();
        if (sae_data_len)
        {
            auth.auth_data = (const u8 *)rt_malloc(sae_data_len);
            memset((void *)auth.auth_data, 0x00, sae_data_len);
            memcpy((void *)auth.auth_data, sae_data, sae_data_len);
            auth.auth_data_len = sae_data_len;
        }
        auth.auth_type = auth_alg;
        ...
        mlan_status rv = wlan_ops_sta_prepare_cmd((mlan_private *)mlan_adap->priv[0], HostCmd_CMD_802_11_TX_FRAME,
                         HostCmd_ACT_GEN_SET, 0, NULL, (void *)dest, &auth);
        if (sae_data_len) rt_free((void *)auth.auth_data);
        if (rv != MLAN_STATUS_SUCCESS)
        {
            return -WM_FAIL;
        }
    ...
    else
    {
        ...
        (void)memcpy(pmgmt_pkt_hdr->wlan_header.addr3, addr, MLAN_MAC_ADDR_LENGTH);
                                ...
                                const t_u16 sae_data_len)
{
    ...
    /*
    pmpriv->auth_flag
    是驱动结构体 mlan_private 中的一个成员变量，
    用于记录当前STA（Station）侧认证（Authentication）流程的状态，即“认证流程是否正在进行中”。
    */
