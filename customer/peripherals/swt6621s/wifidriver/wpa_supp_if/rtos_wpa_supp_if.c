...
void wifi_nxp_wpa_supp_event_proc_auth_resp(void *if_priv, nxp_wifi_event_mlme_t *auth_resp, unsigned int event_len)
{
    ...
    {
        supp_d("%s %d frame_len=%d sizeof(mgmt->u.auth)=%d\n", __func__, __LINE__, frame_len, sizeof(mgmt->u.auth));
...
int wifi_nxp_wpa_supp_scan2(void *if_priv, struct wpa_driver_scan_params *params)
{
    ...
    supp_d("%s %d params->freqs=%p\n", __func__, __LINE__, params->freqs);
//RT_ASSERT(0);
    if (params->freqs)
    {
        for (i = 0; params->freqs[i] && i < WIFI_SCAN_MAX_NUM_CHAN; i++)
        {
            ...
            supp_d("%s %d channels[%d]\n", __func__, __LINE__, i, channels[i]);
        }
    ...
    supp_d("%s %d num_chans=%d\n", __func__, __LINE__, num_chans);
    uint8_t ssid_off = 0;
        ...
        struct wifi_nxp_ctx_rtos *wifi_if_ctx_rtos)
{
    ...
    if (ie_len)
    {
        ...
        //OSA_MemoryFree((void *)ie); //这里可能会造成内存泄漏
    }
    ...
    supp_d("%s %d r=%p pos=%p\n", __func__, __LINE__, r, pos);
    return r;
}
...
int wifi_nxp_wpa_supp_scan_results_get(void *if_priv, struct wpa_scan_results *scan_res2)
{
    ...
    if (scan_res2->res != NULL)
    {
        OSA_MemoryFree(scan_res2->res);
        scan_res2->res = NULL;
    }
    scan_res2->res = (struct wpa_scan_res **)OSA_MemoryAllocate(num * sizeof(struct wpa_scan_res *));
    supp_d("%s %d scan_res2->res = %p\n", __func__, __LINE__, scan_res2->res);
    if (!scan_res2->res)
    {
    ...
    for (i = 0; i < num; i++)
    {
        ...
        supp_d("%s %d num =%d scan_res->ies.ie=%p\n", __func__, __LINE__, num, scan_res.ies.ie);
...
int wifi_nxp_wpa_supp_authenticate(void *if_priv, struct wpa_driver_auth_params *params, struct wpa_bss *curr_bss)
{
    ...
    status = wifi_send_mgmt_auth_request(channel, auth_alg, auth_trans_num, status_code, /*params->bssid*/params->ssid,
                                         (const unsigned char *)pos, len);
    supp_d("%s %d params=%p,params->auth_alg=%d,params->auth_data_len=%d\n", __func__, __LINE__, params, params->auth_alg, params->auth_data_len);

    struct cfg80211_auth_request req = {0};
    req.auth_type = auth_alg;//params->auth_alg;//NL80211_AUTHTYPE_OPEN_SYSTEM;//NL80211_AUTHTYPE_SAE;//NL80211_AUTHTYPE_OPEN_SYSTEM;
    if (params->auth_data_len)
    {
        req.auth_data = params->auth_data;
        req.auth_data_len = params->auth_data_len;
    }
...

{
    ...
    else
    {
        ...
        status =
            wifi_set_key(wifi_if_ctx_rtos->bss_type, is_pairwise, key_idx, key, key_len, seq, seq_len, addr, alg);
