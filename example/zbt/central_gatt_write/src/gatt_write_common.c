...
static K_SEM_DEFINE(sem_sent, 0, 4);
...
static void write_cmd_cb(struct bt_conn *conn, void *user_data)
{
    ...
    delta = k_uptime_get_32() - cycle_stamp;
    ...
    else
    {
        ...
        write_len += len;
        write_rate = ((uint64_t)write_len << 3) * (1U * MSEC_PER_SEC) /
                     delta;
    }
}
